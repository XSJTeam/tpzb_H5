...
const routerBack = {
  computed: {
    handleRoute() {
      ...
      if (t.$store.state.handleBackRouteName.length > 0 && t.$options.name == t.$store.state.handleBackRouteName[0].$options
        .name) { //必须为当前监听页面
  ...
  watch: {
    handleRoute(newV) { //返回键触发
      ...
      if (!newV && (length > 0 && (t.$options.name == t.$store.state.handleBackRouteName[0].$options.name))) { //每次都从第一个监听页面开始	
        console.log("routerback当前路由:" + t.$options.name + " " + t.$store.state.handleBackRouteName[0].$options.name +
          " " + newV)
        ...
        for (let i = 0; i < length; i++) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            if (curRouter[hiddenAttr[curName][i].name]) { //找到当前为true的监听对象
              ...
              console.log("handleRoute Back " + hiddenAttr[curName][i].name + ":" + t.$store.state.handleRoute + " " +
                curName)
  ...
  methods: {
    removePopstateListener() {
      ...
      t.$store.dispatch("changeHandleBackRouteName", {
        type: "del",
        routerName: t
      });
      if (t.isListener) {
    ...
    addPopstateListener() {
      ...
      if (window.history && window.history.pushState && !t.isListener) {
        ...
        if (length > 0) {
          ...
          if (!flag) {
            for (let j = 0; j < length; j++) {
              ...
              if (!flag) {
                for (let i = 0; i < len; i++) {
                  console.log("检测:" + curName + " " + hiddenAttr[curName][i].name + ":" + curRouter[hiddenAttr[
                    curName][i].name])
